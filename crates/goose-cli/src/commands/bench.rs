...
pub async fn handle_terminal_bench_run(options: TerminalBenchRunOptions) -> Result<()> {
    ...
    let eval_dir = repo_root.join("evals/terminal-bench");
    if !eval_dir.exists() {
        bail!(
            "Terminal Bench runner expects a Goose source checkout with {} present",
            eval_dir.display()
        );
    }

    let config = build_terminal_bench_config(&options, &repo_root)?;
    let config_dir = options
        .config_dir
        .clone()
        .unwrap_or_else(|| default_config_dir(&repo_root));
    ...
    command.env("PYTHONPATH", pythonpath_with_eval_dir(&eval_dir));
...
fn build_terminal_bench_config(
    ...
    Ok(json!({
        "job_name": job_name,
        "jobs_dir": options.jobs_dir.clone().unwrap_or_else(|| default_jobs_dir(repo_root)).to_string_lossy(),
...
fn default_jobs_dir(repo_root: &Path) -> PathBuf {
    repo_root.join("target/bench/terminal-bench/jobs")
}

fn default_config_dir(repo_root: &Path) -> PathBuf {
    repo_root.join("target/bench/terminal-bench/configs")
}
...
fn pythonpath_with_eval_dir(eval_dir: &Path) -> String {
    let eval_dir = eval_dir.to_string_lossy();
    match std::env::var("PYTHONPATH") {
        Ok(existing) if !existing.is_empty() => {
            format!("{eval_dir}{}{existing}", pythonpath_separator())
        }
        _ => eval_dir.into_owned(),
    }
}
...
mod tests {
    ...
    fn builds_terminal_bench_config() {
        ...
        let config = build_terminal_bench_config(&options, Path::new("/repo")).unwrap();
    ...
    fn rejects_invalid_terminal_bench_inputs() {
        ...
        assert!(build_terminal_bench_config(&options, Path::new("/repo")).is_err());
